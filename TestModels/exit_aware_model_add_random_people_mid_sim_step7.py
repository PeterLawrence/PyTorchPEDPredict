...
def move_toward_exits(current_frame, distance_field, max_movement=1.5, min_distance=2.0):
    ...
    for i, (x, y) in enumerate(current_positions):
        ...
        # Check distance to other people (collision avoidance)
        too_close = False
        for j, (other_x, other_y) in enumerate(current_positions):
            if i != j:
                dist_to_other = np.sqrt((new_x - other_x)**2 + (new_y - other_y)**2)
                if dist_to_other < min_distance:
                    # Adjust movement to maintain spacing
                    # Add perpendicular component to avoid collision
                    perp_dx = -(dy / (distance + 0.001))  # Perpendicular to gradient
                    perp_dy = dx / (distance + 0.001)
                    
                    # Which direction to dodge?
                    cross = (new_x - other_x) * perp_dx + (new_y - other_y) * perp_dy
                    if cross < 0:
                        perp_dx = -perp_dx
                        perp_dy = -perp_dy
                    
                    # Blend original movement with avoidance
                    dx = 0.6 * dx + 0.4 * perp_dx * max_movement
                    dy = 0.6 * dy + 0.4 * perp_dy * max_movement
                    new_x = x + dx
                    new_y = y + dy
                    too_close = True
                    break
        ...
        if not at_exit:
            ...
            position_occupied = any(
                (new_x == px and new_y == py) for px, py in new_positions
            )
            
            if position_occupied:
                # Keep at current position if new position is taken
...
def predict_evacuation(model, initial_frame, num_steps=100, device='cpu'):
    """Predict with exit-aware model and collision avoidance"""
    model.eval()
    ...
    with torch.no_grad():
        for step in range(num_steps):
            ...
            # Check for premature disappearance
            if people_remaining < prev_people_count:
                # Check if people disappeared from middle of room
                disappeared_positions = []
                current_pos = find_people_positions(current_np)
                new_pos = find_people_positions(new_frame)
                
                for cx, cy in current_pos:
                    # Check if this person still exists in new frame
                    still_exists = any(
                        np.sqrt((cx - nx)**2 + (cy - ny)**2) < 3 
                        for nx, ny in new_pos
                    )
                    if not still_exists:
                        # Check if they were near an exit (within 3 pixels of boundary)
                        near_exit = (cx <= 3 or cx >= 46 or cy <= 3 or cy >= 46)
                        if not near_exit:
                            print(f"  ⚠ Warning: Person at ({cx}, {cy}) disappeared prematurely (not at exit)")
                            disappeared_positions.append((cx, cy))
                
                # If people disappeared prematurely, keep them in current position
